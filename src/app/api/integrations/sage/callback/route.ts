// Sage Callback Route - Handle OAuth2 callback
import { NextRequest, NextResponse } from 'next/server';
import { handleCallback } from '@/lib/integrations/sage/service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (existingIntegration) {
      ...
      await db.integration.update({
        ...
        data: {
          ...
          disconnectedAt: null,
          externalId: result.businessId,
          externalName: result.businessName,
        },
      });
    } else {
    ...
    const protocol = request.headers.get('x-forwarded-proto') || 'https';
    return NextResponse.redirect(
      `${protocol}://${host}/settings/integrations?success=sage_connected`
    );
  } catch (error) {
    ...
    const protocol = request.headers.get('x-forwarded-proto') || 'https';
    return NextResponse.redirect(
      `${protocol}://${host}/settings/integrations?error=${encodeURIComponent(error instanceof Error ? error.message : 'Failed to connect Sage')}`
    );
  }
}
