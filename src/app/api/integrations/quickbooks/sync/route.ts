// QuickBooks Sync Route - Trigger data synchronization
...
import { 
  fetchInvoices, 
  fetchClients, 
  syncInvoices as syncInvoicesService,
  syncClients as syncClientsService,
} from '@/lib/integrations/quickbooks/service';
  ...
  try {
    ...
    try {
      ...
      const integrationData = {
        ...
        type: integration.type as 'quickbooks',
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to get QuickBooks status' },
      { status: 500 }
    );
  }
}
